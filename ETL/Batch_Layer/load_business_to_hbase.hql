...
CREATE EXTERNAL TABLE qi_yelp_business_not_orc(
    business_id String,
    name String,
    categories String,
    review_count int, 
...
LOAD DATA INPATH 'hdfs:///user/qizhu/output/json_business_table' OVERWRITE INTO TABLE qi_yelp_business_not_orc;

CREATE TABLE qi_yelp_business_hbase_sync(
    business_id String,
    name String,
    categories String,
    review_count int, 
...
TBLPROPERTIES ("hbase.table.name" = "qi_yelp_business_hive_sync");

INSERT OVERWRITE TABLE qi_yelp_business_hbase_sync SELECT * FROM qi_yelp_business_not_orc;
