json_review_raw_row = LOAD 'hdfs:///user/qizhu/input/yelp_training_set_review.json' USING JsonLoader('votes:(funny:int, useful:int, cool:int), user_id:chararray, review_id:chararray, stars:int, date:chararray, text:chararray, type:chararray, business_id:chararray');
...
STORE json_review_row_2 INTO 'hdfs:///user/qizhu/output/json_review_table' USING PigStorage('\u0001');

-- STORE json_review_row_2 INTO 'hbase://qi_yelp_review'
...
-- INSERT OVERWRITE TABLE qi_yelp_review_hbase_sync SELECT * FROM qi_yelp_review_not_orc;
